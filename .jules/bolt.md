...
**Learning:** Controllers often use `SELECT *` to fetch data for list views, retrieving significantly more columns than required by the view template (e.g., full address details for a simple table).
**Action:** Explicitly select only the columns used in the view to reduce memory usage and data transfer.
